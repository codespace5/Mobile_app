...
const styles = StyleSheet.create({
  ...
  BottomTabViewSty: {
    position: "absolute",
    // bottom: Device.hasNotch() || isIphoneX() ? 30 : 0,
    bottom: 0,
    left: 0,
    right: 0,
...
class Home extends Component {
  ...
  componentDidMount = () => {
    ...
    SplashScreen.hide();
    this.getAdvTimeOut(); // Enable in live
  ...
  getAdvTimeOut = () => {
    ...
    getApiData(settings.endpoints.adv_Timeout, "get", {}, null)
      .then((response) => {
        if (response.success && _.isObject(response.data)) {
          const adUnitList = response.data.admob_ids;
          const finalObj = {};
          if (!isEmpty(adUnitList)) {
            map(adUnitList, (v, k) => {
              finalObj[v.page_title] = {
                android: v.android_id,
                ios: v.ios_id,
              };
            });
          }
          setAdType({
            ...adType,
            Reward_timeOut: response?.data?.video_timeout?.rewarded_timeout,
            Intertial_timeOut: response?.data?.video_timeout?.intertial_timeout,
            admobUnitIdList: finalObj,
          });
        }
      })
      .catch((err) => {
  ...
  onDidFocus = (payload) => {
    const { isHomeFocused, playVideos } = this.props;
    playVideos(true);
    isHomeFocused(true, true);
  ...
  setAfterLogin = (responseJson) => {
    console.log("Home -> responseJson23", responseJson);
    const {
      navigation,
    ...
    setForceLoad();
    if (responseJson.data.intro_redirect === "1") {
      this.goto("SigninName", responseJson.data);
    } else {
      setTimeout(() => {
        playVideos(true);
      }, 500);
    ...
    return (
      <View style={[styles.BottomTabViewSty]}>
        <TouchableOpacity activeOpacity={0.8} style={styles.BottomTabView}>
          <Icon name="Home-fill" style={styles.TabIconSty} />
        ...
        >
          <Icon name="Trophy-outline" style={styles.TabIconSty} />
          <Text numberOfLines={1} style={styles.BottomTabTitle}>
  ...
  renderItem = ({ item, index }, cTab) => {
    ...
    return (
      <CVideo
        ...
        ShowSearchIcon
        type="home"
        // type={this.props.screenName === "home" ? "home" : "top_100"}
