...
class COfficialRules extends Component {
  constructor(props) {
    super(props);
    this.state = {
      visible: false,
  ...
  getData = async () => {
    try {
      const TermsanduseData = await getCmsData('winmtc-official-rules');
          ...
          <View style={styles.OtherMainView}>
            <View style={styles.HeaderViewSty}>
              <Text numberOfLines={1} style={[common.headerTitle]}>
                {trans('COfficialRules_modal_title')}
              </Text>
              <TouchableOpacity
                activeOpacity={0.8}
...
COfficialRules.propTypes = {
  authActions: PropTypes.objectOf(PropTypes.any),
};

COfficialRules.defaultProps = {
  authActions: {},
};

export default COfficialRules;
